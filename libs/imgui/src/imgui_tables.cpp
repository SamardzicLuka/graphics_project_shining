...
auto
ImGui::TableFindByID(ImGuiID id) -> ImGuiTable*
{
                    ...
                    float inner_width) -> bool
{
  ...
  if (use_child_window) {
    ...
    BeginChildEx(
      name, instance_id, table->OuterRect.GetSize(), false, child_flags);
...
// extending
static auto
TableFixColumnFlags(ImGuiTable* table, ImGuiTableColumnFlags flags)
  -> ImGuiTableColumnFlags
{
  ...
  IM_ASSERT(ImIsPowerOfTwo(
    flags & ImGuiTableColumnFlags_WidthMask_)); // Check that only 1 of each set
                                                // is used.
...
ImGui::TableUpdateLayout(ImGuiTable* table)
{
  ...
  int count_fixed =
    0; // Number of columns that have fixed sizing policy (not stretched sizing
       // policy) (this is NOT the opposite of count_resizable!)
  int count_resizable = 0; // Number of columns the user can resize (this is NOT
                           // the opposite of count_fixed!)
  ...
  float sum_width_fixed_requests =
    0.0f; // Sum of all width for fixed and auto-resize columns, excluding width
          // contributed by Stretch columns.
  ...
  for (int column_n = 0; column_n < table->ColumnsCount; column_n++) {
    ...
    // Adjust flags: default width mode + weighted columns are not allowed when
    // auto extending
    // FIXME-TABLE: Clarify why we need to do this again here and not just in
    // TableSetupColumn()
    ...
    // TableGetSortSpecs() but has sorting enabled or varying sorting flags, we
    // still want the sorting arrows to honor those flags.
  ...
  for (int column_n = 0; column_n < table->ColumnsCount; column_n++) {
    ...
    if (column->Flags & ImGuiTableColumnFlags_WidthStretch) {
      ...
      // [Resize Rule 2] Resizing from right-side of a stretch column preceding
      // a fixed column needs to forward resizing to left-side of fixed column.
    ...
    // is at least one Stretch column (see comments in TableResizeColumn())
    if (column->NextEnabledColumn == -1 &&
        table->LeftMostStretchedColumnDisplayOrder != -1)
  ...
  for (int order_n = 0; order_n < table->ColumnsCount; order_n++) {
    ...
    // is detrimental to visibility in very-small column.
    ...
    // FIXME-TABLE: IsVisible == false is disabled because it effectively means
    // not submitting will reduces contents width which is fed to
    ...
    //     if ((column->AutoFitQueue & 1) == 0 && (column->CannotSkipItemsQueue
    //     & 1) == 0)
    //         column->IsSkipItems = true;
...
ImGui::TableSetColumnWidth(int column_n, float width)
{
  ...
  // Stretch columns. When forwarding resize from Wn| to Fn+1| we need to be
  // considerate of the _NoResize flag on Fn+1.
  ...
  // - W1 F2 F3  resize from F2|          --> FIXME should resize F2, F3 and not
  // have effect on W1 (Stretch columns are _before_ the Fixed column).
                        ...
                        ImGuiID user_id)
{
  ...
  if (table->IsInitializing) {
    ...
    if (flags & ImGuiTableColumnFlags_DefaultSort &&
      ...
      column->SortOrder = 0; // Multiple columns using _DefaultSort will be
...
ImGui::TableEndCell(ImGuiTable* table)
{
  ...
  if (table->RowFlags & ImGuiTableRowFlags_Headers)
    p_max_pos_x =
      &column->ContentMaxXHeadersUsed; // Useful in case user submit contents in
                                       // header row that is not a TableHeader()
                                       // call
  else
...
auto
ImGui::TableSetColumnIndex(int column_n) -> bool
{
  ImGuiContext& g = *GImGui;
  ImGuiTable* table = g.CurrentTable;
  if (!table)
...
auto
ImGui::TableGetColumnName(int column_n) -> const char*
{
  ImGuiContext& g = *GImGui;
  ImGuiTable* table = g.CurrentTable;
  if (!table)
...
auto
ImGui::TableGetColumnFlags(int column_n) -> ImGuiTableColumnFlags
{
  ImGuiContext& g = *GImGui;
  ImGuiTable* table = g.CurrentTable;
  if (!table)
...
void
ImGui::TableSetColumnIsEnabled(int column_n, bool hidden)
{
  ImGuiContext& g = *GImGui;
  ImGuiTable* table = g.CurrentTable;
  IM_ASSERT(table != nullptr && table->IsLayoutLocked == false);
...
//   outer border.
auto
ImGui::TableGetCellBgRect(const ImGuiTable* table, int column_n) -> ImRect
{
...
auto
ImGui::TableGetColumnName(const ImGuiTable* table, int column_n) -> const char*
{
  const ImGuiTableColumn* column = &table->Columns[column_n];
...
auto
ImGui::TableGetColumnResizeID(const ImGuiTable* table,
                              int column_n,
                              int instance_no) -> ImGuiID
{
...
auto
ImGui::TableGetHoveredColumn() -> int
{
  ImGuiContext& g = *GImGui;
  ImGuiTable* table = g.CurrentTable;
  if (!table)
...
void
ImGui::TableSetBgColor(ImGuiTableBgTarget bg_target, ImU32 color, int column_n)
{
  ImGuiContext& g = *GImGui;
  ImGuiTable* table = g.CurrentTable;
...
ImGui::TableMergeDrawChannels(ImGuiTable* table)
{
  ...
  if (merge_group_mask != 0) {
    ...
    g.DrawChannelsTempMergeBuffer.resize(
      splitter->_Count -
      LEADING_DRAW_CHANNELS); // Use shared temporary storage so the allocation
                              // gets amortized
...
void
ImGui::TableFixColumnSortDirection(ImGuiTableColumn* column)
{
  // Initial sort state
...
auto
ImGui::TableGetHeaderRowHeight() -> float
{
...
ImGui::TableDrawContextMenu(ImGuiTable* table)
{
  ...
  const int column_n = (table->ContextPopupColumn >= 0 &&
                        table->ContextPopupColumn < table->ColumnsCount)
                         ...
                         : -1;
  ImGuiTableColumn* column =
    (column_n != -1) ? &table->Columns[column_n] : nullptr;
...
static auto
TableSettingsCalcChunkSize(int columns_count) -> size_t
{
...
auto
ImGui::TableSettingsCreate(ImGuiID id, int columns_count) -> ImGuiTableSettings*
{
  ImGuiContext& g = *GImGui;
...
auto
ImGui::TableSettingsFindByID(ImGuiID id) -> ImGuiTableSettings*
{
...
auto
ImGui::TableGetBoundSettings(ImGuiTable* table) -> ImGuiTableSettings*
{
...
void
ImGui::TableSettingsInstallHandler(ImGuiContext* context)
{
...
static auto
GetDraggedColumnOffset(ImGuiOldColumns* columns, int column_index) -> float
{
...
auto
ImGui::GetColumnOffset(int column_index) -> float
{
  ImGuiWindow* window = GetCurrentWindowRead();
...
GetColumnWidthEx(ImGuiOldColumns* columns,
                 int column_index,
                 bool before_resize = false) -> float
{
  if (column_index < 0)
    column_index = columns->Current;
...
auto
ImGui::GetColumnWidth(int column_index) -> float
{
  ImGuiContext& g = *GImGui;
  ImGuiWindow* window = g.CurrentWindow;
  ImGuiOldColumns* columns = window->DC.CurrentColumns;
...
auto
ImGui::FindOrCreateColumns(ImGuiWindow* window, ImGuiID id) -> ImGuiOldColumns*
{
...
auto
ImGui::GetColumnsID(const char* str_id, int columns_count) -> ImGuiID
{
