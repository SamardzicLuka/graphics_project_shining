...
InputTextCalcTextSizeW(const ImWchar* text_begin,
                       ...
                       bool stop_on_new_line = false) -> ImVec2;
...
auto
ImGui::Button(const char* label, const ImVec2& size_arg) -> bool
{
...
auto
ImGui::ArrowButton(const char* str_id, ImGuiDir dir) -> bool
{
...
auto
ImGui::CloseButton(ImGuiID id, const ImVec2& pos) -> bool //, float size)
{
  ImGuiContext& g = *GImGui;
...
auto
ImGui::CollapseButton(ImGuiID id, const ImVec2& pos) -> bool
{
  ImGuiContext& g = *GImGui;
  ImGuiWindow* window = g.CurrentWindow;
...
auto
ImGui::GetWindowScrollbarID(ImGuiWindow* window, ImGuiAxis axis) -> ImGuiID
{
...
auto
ImGui::GetWindowScrollbarRect(ImGuiWindow* window, ImGuiAxis axis) -> ImRect
{
...
ImGui::ScrollbarEx(const ImRect& bb_frame,
                   ...
                   ImDrawCornerFlags rounding_corners) -> bool
{
  ImGuiContext& g = *GImGui;
  ImGuiWindow* window = g.CurrentWindow;
  if (window->SkipItems)
...
template<typename T>
auto
ImGui::CheckboxFlagsT(const char* label, T* flags, T flags_value) -> bool
{
...
auto
ImGui::CheckboxFlags(const char* label, int* flags, int flags_value) -> bool
{
...
auto
ImGui::RadioButton(const char* label, bool active) -> bool
{
  ImGuiWindow* window = GetCurrentWindow();
  if (window->SkipItems)
    return false;
...
ImGui::SplitterBehavior(const ImRect& bb,
                        ...
                        float min_size2,
                        float hover_extend,
                        float hover_visibility_delay) -> bool
{
  ImGuiContext& g = *GImGui;
  ImGuiWindow* window = g.CurrentWindow;
...
ShrinkWidthItemComparer(const void* lhs, const void* rhs) -> int
{
...
static auto
Items_ArrayGetter(void* data, int idx, const char** out_text) -> bool
{
...
static auto
Items_SingleStringGetter(void* data, int idx, const char** out_text) -> bool
{
...
ImGui::Combo(const char* label,
             ...
             void* data,
             int items_count,
             int popup_max_height_in_items) -> bool
{
  ImGuiContext& g = *GImGui;
...
static auto
PatchFormatStringFloatToInt(const char* fmt) -> const char*
{
  ...
  const char* fmt_end = ImParseFormatFindEnd(
    fmt_start); // Find end of format specifier, which itself is an exercise of
                // confidence/recklessness (because snprintf is dependent on
                // libc or user).
...
auto
ImGui::DataTypeGetInfo(ImGuiDataType data_type) -> const ImGuiDataTypeInfo*
{
...
template<typename T>
static auto
DataTypeCompareT(const T* lhs, const T* rhs) -> int
{
  if (*lhs < *rhs)
    return -1;
  if (*lhs > *rhs)
...
template<typename T>
static auto
DataTypeClampT(T* v, const T* v_min, const T* v_max) -> bool
{
...
static auto
GetMinimumStepAtDecimalPrecision(int decimal_precision) -> float
{
...
static auto
ImAtoi(const char* src, TYPE* output) -> const char*
{
  int negative = 0;
  if (*src == '-') {
    negative = 1;
                  ...
                  float power) -> bool
{
  ...
  if (power != 1.0f) {
    ...
    IM_ASSERT(p_min != nullptr &&
              p_max != nullptr); // When using a power curve the drag needs to
                                 // have known bounds
    drag_flags |=
...
ImGui::ScaleRatioFromValueT(ImGuiDataType data_type,
                            ...
                            bool is_logarithmic,
                            float logarithmic_zero_epsilon,
                            float zero_deadzone_halfsize) -> float
{
  if (v_min == v_max)
    return 0.0f;
  IM_UNUSED(data_type);
  ...
  if (is_logarithmic) {
             ...
             0.0f) // Range crosses zero, so split into two portions
    {
      float zero_point_center =
        ...
        ((float)v_max -
         (float)
           v_min); // The zero point in parametric space.  There's an argument
...
ImGui::ScaleValueFromRatioT(ImGuiDataType data_type,
                            ...
                            bool is_logarithmic,
                            float logarithmic_zero_epsilon,
                            float zero_deadzone_halfsize) -> TYPE
{
  if (v_min == v_max)
    return (TYPE)0.0f;
  const bool is_decimal =
...
// start with '%'
auto
ImParseFormatFindStart(const char* fmt) -> const char*
{
  while (char c = fmt[0]) {
    if (c == '%' && fmt[1] != '%')
      return fmt;
...
auto
ImParseFormatFindEnd(const char* fmt) -> const char*
{
...
auto
ImParseFormatPrecision(const char* fmt, int default_precision) -> int
{
...
auto
ImGui::InputInt2(const char* label, int v[2], ImGuiInputTextFlags flags) -> bool
{
  return InputScalarN(
...
auto
ImGui::InputInt3(const char* label, int v[3], ImGuiInputTextFlags flags) -> bool
{
  return InputScalarN(
...
auto
ImGui::InputInt4(const char* label, int v[4], ImGuiInputTextFlags flags) -> bool
{
  return InputScalarN(
...
static auto
STB_TEXTEDIT_STRINGLEN(const STB_TEXTEDIT_STRING* obj) -> int
{
  return obj->CurLenW;
}
static auto
STB_TEXTEDIT_GETCHAR(const STB_TEXTEDIT_STRING* obj, int idx) -> ImWchar
{
...
static auto
STB_TEXTEDIT_KEYTOTEXT(int key) -> int
{
...
static auto
is_separator(unsigned int c) -> bool
{
  return ImCharIsBlankW(c) || c == ',' || c == ';' || c == '(' || c == ')' ||
         c == '{' || c == '}' || c == '[' || c == ']' || c == '|';
}
static auto
is_word_boundary_from_right(STB_TEXTEDIT_STRING* obj, int idx) -> int
{
...
static auto
STB_TEXTEDIT_MOVEWORDLEFT_IMPL(STB_TEXTEDIT_STRING* obj, int idx) -> int
{
  idx--;
...
#else
static auto
STB_TEXTEDIT_MOVEWORDRIGHT_IMPL(STB_TEXTEDIT_STRING* obj, int idx) -> int
{
  idx++;
  int len = obj->CurLenW;
                   ...
                   void* callback_user_data) -> bool
{
  ...
  const bool init_changed_specs =
    (state != nullptr && state->Stb.single_line != !is_multiline);
  ...
  if ((init_state && g.ActiveId != id) || init_changed_specs) {
    ...
    state->TextW.resize(
      buf_size + 1); // wchar count <= UTF-8 count. we use +1 to make sure that
                     // .Data is always pointing to at least an empty string.
    state->TextA.resize(0);
    state->TextAIsValid =
      false; // TextA is not valid yet (we will display buf until then)
    state->CurLenW =
      ImTextStrFromUtf8(state->TextW.Data, buf_size, buf, nullptr, &buf_end);
    state->CurLenA =
      (int)(buf_end -
  ...
  if (g.ActiveId == id) {
    ...
    if (apply_edit_back_to_user_buffer) {
      ...
      if (!is_readonly) {
        ...
        ImTextStrToUtf8(
          state->TextA.Data, state->TextA.Size, state->TextW.Data, nullptr);
      }
  ...
  const char* buf_display_end =
    nullptr; // We have specialized paths below for setting the length
    ...
    if (render_selection) {
      ...
      for (const ImWchar* p = text_selected_begin; p < text_selected_end;) {
        ...
        if (rect_pos.y < clip_rect.y) {
          ...
          ImVec2 rect_size =
            InputTextCalcTextSizeW(p, text_selected_end, &p, nullptr, true);
          if (rect_size.x <= 0.0f)
...
auto
ImGui::TreeNode(const char* str_id, const char* fmt, ...) -> bool
{
  va_list args;
  va_start(args, fmt);
...
auto
ImGui::TreeNode(const void* ptr_id, const char* fmt, ...) -> bool
{
  va_list args;
  va_start(args, fmt);
...
auto
ImGui::TreeNodeV(const char* str_id, const char* fmt, va_list args) -> bool
{
  return TreeNodeExV(str_id, 0, fmt, args);
}

auto
ImGui::TreeNodeV(const void* ptr_id, const char* fmt, va_list args) -> bool
{
  return TreeNodeExV(ptr_id, 0, fmt, args);
}

auto
ImGui::TreeNodeEx(const char* label, ImGuiTreeNodeFlags flags) -> bool
{
  ImGuiWindow* window = GetCurrentWindow();
  if (window->SkipItems)
    return false;
...
auto
ImGui::TreeNodeBehaviorIsOpen(ImGuiID id, ImGuiTreeNodeFlags flags) -> bool
{
...
auto
ImGui::CollapsingHeader(const char* label, ImGuiTreeNodeFlags flags) -> bool
{
  ImGuiWindow* window = GetCurrentWindow();
  if (window->SkipItems)
    return false;
...
auto
ImGui::ListBoxHeader(const char* label, const ImVec2& size_arg) -> bool
{
  ImGuiContext& g = *GImGui;
  ImGuiWindow* window = GetCurrentWindow();
  if (window->SkipItems)
               ...
               int height_in_items) -> bool
{
  ...
  clipper.Begin(
    items_count,
    GetTextLineHeightWithSpacing()); // We know exactly our line height here so
                                     // we pass it as a minor optimization, but
...
static auto
Plot_ArrayGetter(void* data, int idx) -> float
{
...
auto
ImGuiMenuColumns::DeclColumns(float w0, float w1, float w2)
  -> float // not using va_arg because they promote float to double
{
  NextWidth = 0.0f;
...
auto
ImGuiMenuColumns::CalcExtraSpace(float avail_w) const -> float
{
...
ImGui::EndMenuBar()
{
  ...
  window->DC.MenuBarOffset.x =
    window->DC.CursorPos.x -
    window->MenuBarRect()
      .Min.x; // Save horizontal position so next append can reuse it. This is
...
auto
ImGui::BeginMenu(const char* label, bool enabled) -> bool
{
  ImGuiWindow* window = GetCurrentWindow();
  if (window->SkipItems)
    return false;
  ...
  if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
    ...
    RenderArrow(
      window->DrawList,
      pos + ImVec2(window->DC.MenuColumns.Pos[2] + extra_w + g.FontSize * 0.30f,
                   0.0f),
      text_col,
      ImGuiDir_Right);
  }
  ...
  if (menu_is_open) {
    ...
    menu_is_open =
      BeginPopupEx(id, flags); // menu_is_open can be 'false' when the popup is
...
static auto
TabBarCalcTabID(ImGuiTabBar* tab_bar, const char* label) -> ImU32;
static auto
TabBarCalcMaxTabWidth() -> float;
static auto
TabBarScrollClamp(ImGuiTabBar* tab_bar, float scrolling) -> float;
static void
TabBarScrollToTab(ImGuiTabBar* tab_bar,
                  ImGuiTabItem* tab,
                  ImGuiTabBarSection* sections);
static auto
TabBarScrollingButtons(ImGuiTabBar* tab_bar) -> ImGuiTabItem*;
static auto
TabBarTabListPopupButton(ImGuiTabBar* tab_bar) -> ImGuiTabItem*;
}
...
static auto IMGUI_CDECL
TabItemComparerBySection(const void* lhs, const void* rhs) -> int
{
...
static auto IMGUI_CDECL
TabItemComparerByBeginOrder(const void* lhs, const void* rhs) -> int
{
  const auto* a = (const ImGuiTabItem*)lhs;
  const auto* b = (const ImGuiTabItem*)rhs;
...
static auto
GetTabBarFromTabBarRef(const ImGuiPtrOrIndex& ref) -> ImGuiTabBar*
{
  ImGuiContext& g = *GImGui;
...
static auto
GetTabBarRefFromTabBar(ImGuiTabBar* tab_bar) -> ImGuiPtrOrIndex
{
  ImGuiContext& g = *GImGui;
...
auto
ImGui::BeginTabBar(const char* str_id, ImGuiTabBarFlags flags) -> bool
{
  ImGuiContext& g = *GImGui;
  ImGuiWindow* window = g.CurrentWindow;
  if (window->SkipItems)
...
// stack.
static auto
ImGui::TabBarCalcTabID(ImGuiTabBar* tab_bar, const char* label) -> ImU32
{
...
static auto
ImGui::TabBarCalcMaxTabWidth() -> float
{
  ImGuiContext& g = *GImGui;
...
auto
ImGui::TabBarFindTabByID(ImGuiTabBar* tab_bar, ImGuiID tab_id) -> ImGuiTabItem*
{
  if (tab_id != 0)
...
static auto
ImGui::TabBarScrollClamp(ImGuiTabBar* tab_bar, float scrolling) -> float
{
  scrolling =
                         ...
                         ImGuiTabBarSection* sections)
{
  ...
  float margin =
    g.FontSize * 1.0f; // When to scroll to make Tab N+1 visible always make a
                       // bit of N visible to suggest more scrolling area (since
...
void
ImGui::TabBarQueueReorder(ImGuiTabBar* tab_bar,
...
auto
ImGui::TabBarProcessReorder(ImGuiTabBar* tab_bar) -> bool
{
...
static auto
ImGui::TabBarScrollingButtons(ImGuiTabBar* tab_bar) -> ImGuiTabItem*
{
  ImGuiContext& g = *GImGui;
  ImGuiWindow* window = g.CurrentWindow;
...
static auto
ImGui::TabBarTabListPopupButton(ImGuiTabBar* tab_bar) -> ImGuiTabItem*
{
  ImGuiContext& g = *GImGui;
  ImGuiWindow* window = g.CurrentWindow;
  ...
  bool open = BeginCombo(
    "##v", nullptr, ImGuiComboFlags_NoPreview | ImGuiComboFlags_HeightLargest);
  PopStyleColor(2);
  ...
  -> bool
{
  ...
  if (ret && !(flags & ImGuiTabItemFlags_NoPushId)) {
    ...
    PushOverrideID(
      tab->ID); // We already hashed 'label' so push into the ID stack directly
...
auto
ImGui::TabItemButton(const char* label, ImGuiTabItemFlags flags) -> bool
{
  ImGuiContext& g = *GImGui;
  ImGuiWindow* window = g.CurrentWindow;
  if (window->SkipItems)
                 ...
                 ImGuiTabItemFlags flags) -> bool
{
  ...
  const ImGuiID close_button_id =
    p_open ? GetIDWithSeed("#CLOSE", nullptr, id) : 0;
  bool just_closed;
  bool text_clipped;
...
auto
ImGui::TabItemCalcSize(const char* label, bool has_close_button) -> ImVec2
{
  ImGuiContext& g = *GImGui;
  ImVec2 label_size = CalcTextSize(label, nullptr, true);
