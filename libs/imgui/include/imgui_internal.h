...
ImTextStrFromUtf8(
  ...
  const char** in_text_remaining = NULL); // return input UTF-8 bytes count
IMGUI_API int
...
IMGUI_API ImU64
ImFileRead(void* data, ImU64 sz, ImU64 count, ImFileHandle file);
IMGUI_API ImU64
ImFileWrite(const void* data, ImU64 sz, ImU64 count, ImFileHandle file);
#else
...
#define ImFloorStd(X)                                                          \
  floorf(X) // We already uses our own ImFloor() { return (float)(int)v }
            // internally so the standard one wrapper is named differently (it's
            // used by e.g. stb_truetype)
...
inline void
ImBitArraySetBitRange(ImU32* arr, int n, int n2)
{
  while (n <= n2) {
    int a_mod = (n & 31);
...
enum ImGuiItemFlags_
{
  ...
  ImGuiItemFlags_MixedValue =
    1
    << 6, // false    // [BETA] Represent a mixed/indeterminate value, generally
...
enum ImGuiItemStatusFlags_
{
  ...
  ImGuiItemStatusFlags_ToggledSelection =
    1 << 3, // Set when Selectable(), TreeNode() reports toggling a selection.
            // We can't report "Selected" because reporting the change allows us
            // to handle clipping with less issues.
...
enum ImGuiTooltipFlags_
{
  ...
  ImGuiTooltipFlags_OverridePreviousTooltip =
    1 << 0 // Override will clear/ignore previously submitted tooltip (defaults
           // to append)
};
...
struct IMGUI_API ImGuiInputTextState
{
  ...
  ImVector<char>
    TextA; // temporary UTF8 buffer for callbacks and other operations. this is
...
struct ImGuiNextItemData
{
  ...
  ImGuiID
    FocusScopeId; // Set by SetNextItemMultiSelectData() (!= 0 signify value has
...
struct ImGuiContext
{
  ...
  ImVector<ImGuiWindow*>
    WindowsFocusOrder; // Windows, sorted in focus order, back to front. (FIXME:
                       // We could only store root windows here! Need to sort
                       // out the Docking equivalent which is RootWindowDockStop
                       // and is unfortunately a little more dynamic)
  ImVector<ImGuiWindow*>
  ...
  ImGuiWindow* HoveredWindow; // Window the mouse is hovering. Will typically
                              // catch mouse inputs.
  ImGuiWindow*
    HoveredRootWindow; // == HoveredWindow ? HoveredWindow->RootWindow : NULL,
                       // merely a shortcut to avoid null test in some
                       // situation.
  ImGuiWindow*
  ...
  ImGuiWindow*
    WheelingWindow; // Track the window we started mouse-wheeling on. Until a
                    // timer elapse or mouse has moved, generally keep scrolling
                    // the same window even if during the course of scrolling
  ...
  bool ActiveIdHasBeenPressedBefore; // Track whether the active id led to a
                                     ...
                                     // branch.
  bool
    ActiveIdHasBeenEditedBefore; // Was the value associated to the widget
  ...
  ImVec2
    ActiveIdClickOffset; // Clicked offset from upper-left corner, if applicable
  ...
  ImDrawList
    ForegroundDrawList; // Last draw list to be rendered. This is where we the
                        // render software mouse cursor (if io.MouseDrawCursor
  ...
  ImGuiTextBuffer
    LogBuffer; // Accumulation buffer when log to clipboard. This is pointer so
...
struct IMGUI_API ImGuiWindowTempData
{
  ...
  ImRect LastItemDisplayRect; // End-user display rect for last item (only valid
                              // if LastItemStatusFlags &
  ...
  int NavLayerActiveMask; // Which layers have been written to (result from
                          // previous frame)
  int NavLayerActiveMaskNext; // Which layers have been written to (accumulator
                              // for current frame)
  ...
  ImVec2
    MenuBarOffset; // MenuBarOffset.x is sort of equivalent of a per-layer
  ...
  int
    FocusCounterRegular; // (Legacy Focus/Tabbing system) Sequential counter,
...
struct IMGUI_API ImGuiWindow
{
  ...
  ImVec2
    ContentSize; // Size of contents/scrollable client area (calculated from the
                 // extents reach of the cursor) from previous frame. Does not
                 // include window decoration or window padding.
  ImVec2
    ContentSizeExplicit;  // Size of contents/scrollable client area explicitly
  ...
  ImGuiWindowTempData
    DC; // Temporary per-window data, reset at the beginning of the frame. This
  ...
  ImRect
    ParentWorkRect; // Backup of WorkRect before entering a container such as
...
struct ImGuiTableColumn
{
  ...
  float
    WorkMinX; // Start position for the frame, currently ~(MinX + CellPaddingX)
  float WorkMaxX;
  float ItemWidth;
  float
    ContentMaxXFrozen; // Contents maximum position for frozen rows (apart from
  ...
  ImGuiTableColumnIdx IndexWithinEnabledSet; // Index within enabled/visible set
                                             // (<= IndexToDisplayOrder)
  ImGuiTableColumnIdx
    PrevEnabledColumn; // Index of prev enabled/visible column within Columns[],
                       // -1 if first enabled/visible column
  ImGuiTableColumnIdx
    NextEnabledColumn; // Index of next enabled/visible column within Columns[],
...
struct ImGuiTable
{
  ...
  ImSpan<ImGuiTableCellData>
    RowCellData; // Point within RawData[]. Store cells background requests for
                 // current row.
  ...
  ImGuiTableColumnIdx AutoFitSingleStretchColumn; // Index of single stretch
                                                  // column requesting auto-fit.
...
IMGUI_API ImVec2
CalcWindowExpectedSize(ImGuiWindow* window);
IMGUI_API bool
IsWindowChildOf(ImGuiWindow* window, ImGuiWindow* potential_parent);
IMGUI_API bool
IsWindowAbove(ImGuiWindow* potential_above, ImGuiWindow* potential_below);
IMGUI_API bool
IsWindowNavFocusable(ImGuiWindow* window);
IMGUI_API ImRect
GetWindowAllowedExtentRect(ImGuiWindow* window);
IMGUI_API void
...
IMGUI_API void
AddContextHook(ImGuiContext* ctx, const ImGuiContextHook* hook);
IMGUI_API void
CallContextHooks(ImGuiContext* ctx, ImGuiContextHookType type);
...
IMGUI_API ImGuiID
GetIDWithSeed(const char* str_id_begin, const char* str_end, ImGuiID seed);
...
IMGUI_API bool
ItemHoverable(const ImRect& bb, ImGuiID id);
IMGUI_API bool
IsClippedEx(const ImRect& bb, ImGuiID id, bool clip_even_when_logged);
IMGUI_API void
SetLastItemData(ImGuiWindow* window,
                ImGuiID item_id,
...
IMGUI_API void
PushMultiItemsWidths(int components, float w_full);
IMGUI_API void
...
IMGUI_API float
GetNavInputAmount(ImGuiNavInput n, ImGuiInputReadMode mode);
IMGUI_API ImVec2
...
ActivateItem(
  ImGuiID id); // Remotely activate a button, checkbox, tree node etc. given its
               // unique ID. activation is queued and processed on the next
               // frame when the item is encountered again.
IMGUI_API void
SetNavID(ImGuiID id, int nav_layer, ImGuiID focus_scope_id);
IMGUI_API void
SetNavIDWithRectRel(ImGuiID id,
                    int nav_layer,
...
inline bool
IsActiveIdUsingNavInput(ImGuiNavInput input)
{
  ImGuiContext& g = *GImGui;
  return (g.ActiveIdUsingNavInputMask & (1 << input)) != 0;
}
inline bool
IsActiveIdUsingKey(ImGuiKey key)
{
  ImGuiContext& g = *GImGui;
  IM_ASSERT(key < 64);
...
inline bool
IsNavInputTest(ImGuiNavInput n, ImGuiInputReadMode rm)
{
  return (GetNavInputAmount(n, rm) > 0.0f);
}
IMGUI_API ImGuiKeyModFlags
...
IMGUI_API void
TableSetColumnIsEnabled(int column_n, bool hidden);
IMGUI_API void
...
IMGUI_API void
TableDrawContextMenu(ImGuiTable* table);
IMGUI_API void
...
IMGUI_API void
TableFixColumnSortDirection(ImGuiTableColumn* column);
IMGUI_API void
...
IMGUI_API ImGuiID
TableGetColumnResizeID(const ImGuiTable* table,
                       int column_n,
                       int instance_no = 0);
IMGUI_API void
...
IMGUI_API void
TableSettingsInstallHandler(ImGuiContext* context);
IMGUI_API ImGuiTableSettings*
...
IMGUI_API void
TabBarQueueReorder(ImGuiTabBar* tab_bar, const ImGuiTabItem* tab, int dir);
IMGUI_API bool
...
inline void
RenderArrow(ImVec2 pos, ImGuiDir dir, float scale = 1.0f)
{
  ImGuiWindow* window = GetCurrentWindow();
  RenderArrow(window->DrawList, pos, GetColorU32(ImGuiCol_Text), dir, scale);
}
inline void
RenderBullet(ImVec2 pos)
{
  ImGuiWindow* window = GetCurrentWindow();
...
IMGUI_API void
SeparatorEx(ImGuiSeparatorFlags flags);
...
DebugNodeDrawList(ImGuiWindow* window,
                  ...
                  const char* label);
IMGUI_API void
DebugNodeDrawCmdShowMeshAndBoundingBox(ImGuiWindow* window,
                                       const ImDrawList* draw_list,
...
extern void
ImGuiTestEngineHook_IdInfo(ImGuiContext* ctx,
                           ImGuiDataType data_type,
                           ImGuiID id,
                           const void* data_id);
extern void
