...
SetWindowFontScale(
  float
    scale); // set font scale. Adjust IO.FontGlobalScale if you want to scale
...
PushItemWidth(
  float
    item_width); // push width of items for common large "item+label" widgets.
                 // >0.0f: width in pixels, <0.0f align xx pixels to the right
                 // of window (so -FLT_MIN always align width to the right
...
IMGUI_API void
NewLine(); // undo a SameLine() or force a new line when in an horizontal-layout
           // context.
IMGUI_API void
...
IMGUI_API ImVec2
GetCursorPos(); // cursor position in window coordinates (relative to window
                // position)
IMGUI_API float
...
OpenPopupOnItemClick(
  ...
  ImGuiPopupFlags popup_flags =
    1); // helper to open popup when clicked on last item. return true when just
...
BeginPopupContextItem(
  const char* str_id = NULL,
  ImGuiPopupFlags popup_flags =
    1); // open+begin popup when clicked on last item. if you can pass a NULL
        // str_id only if the previous item had an id. If you want to use that
        // on a non-interactive item such as Text() you need to pass in an
...
TableNextRow(
  ...
  float row_min_height = 0.0f); // append into the first cell of a new row.
IMGUI_API bool
...
SetStateStorage(
  ImGuiStorage*
    tree); // replace current window storage with our own (if you want to
...
CalcListClipping(
  ...
  int*
    out_items_display_end); // calculate coarse clipping for large list of
...
IsMouseDoubleClicked(
  ImGuiMouseButton
    button); // did mouse button double-clicked? (note that a double-click will
...
DebugCheckVersionAndDataLayout(
  ...
  size_t sz_vec4,
  size_t sz_vert,
  size_t sz_idx); // This is called by IMGUI_CHECKVERSION() macro.
...
enum ImGuiTabItemFlags_
{
  ...
  ImGuiTabItemFlags_UnsavedDocument =
    1 << 0, // Append '*' to title without affecting the ID, as a convenience to
            // avoid using the ### operator. Also: tab is selected on closure
            // and closure is deferred by one frame to allow code to undo it
            // without flicker.
...
enum ImGuiTableFlags_
{
  ...
  ImGuiTableFlags_MultiSortable =
    1 << 4, // Allow sorting on multiple columns by holding Shift
            // (sort_specs_count may be > 1). Call TableGetSortSpecs() to obtain
            // sort specs.
  ImGuiTableFlags_NoSavedSettings =
    1 << 5, // Disable persisting columns order, width and sort settings in the
            // .ini file.
  ImGuiTableFlags_ContextMenuInBody =
    1
    << 6, // Right-click on columns body/contents will display table context
  ...
  ImGuiTableFlags_BordersInnerH = 1
                                  << 8, // Draw horizontal borders between rows.
  ...
  ImGuiTableFlags_NoBordersInBody =
    1 << 12, // Disable vertical borders in columns Body (borders will always
             // appears in Headers).
  ...
  ImGuiTableFlags_NoKeepColumnsVisible =
    1 << 19, // Disable keeping column always minimally visible when ScrollX is
             // off and table gets too small.
  ImGuiTableFlags_PreciseWidths =
    1 << 20, // Disable distributing remainder width to stretched columns (width
  ...
  ImGuiTableFlags_NoClip =
    1 << 21, // Disable clipping rectangle for every individual columns (reduce
  ...
  ImGuiTableFlags_NoPadInnerX =
    1 << 24, // Disable inner padding between columns (double inner padding if
...
enum ImGuiTableColumnFlags_
{
  ...
  ImGuiTableColumnFlags_WidthStretch =
    1
    << 2, // Column will stretch. Preferable with horizontal scrolling disabled
  ...
  ImGuiTableColumnFlags_WidthFixed =
    1 << 3, // Column will not stretch. Preferable with horizontal scrolling
            // enabled (default if table sizing policy is _ColumnsWidthFixed and
            // table is resizable).
  ...
  ImGuiTableColumnFlags_PreferSortAscending =
    1 << 13, // Make the initial sort direction Ascending when first sorting on
             // this column (default).
  ImGuiTableColumnFlags_PreferSortDescending =
    1 << 14, // Make the initial sort direction Descending when first sorting on
             // this column.
  ImGuiTableColumnFlags_IndentEnable =
    1 << 15, // Use current Indent value when entering cell (default for column
             // 0).
...
enum ImGuiFocusedFlags_
{
  ...
  ImGuiFocusedFlags_RootWindow =
    1 << 1, // IsWindowFocused(): Test from root window (top most parent of the
            // current hierarchy)
...
enum ImGuiConfigFlags_
{
  ...
  ImGuiConfigFlags_NavEnableGamepad =
    1 << 1, // Master gamepad navigation enable flag. This is mostly to instruct
            // your imgui backend to fill io.NavInputs[]. Backend also needs to
...
struct ImVector
{
  ...
  inline T* erase(const T* it)
  {
    ...
    memmove(
      Data + off, Data + off + 1, ((size_t)Size - (size_t)off - 1) * sizeof(T));
    Size--;
    return Data + off;
  }
...
struct ImGuiStyle
{
  ...
  ImVec2
    ItemInnerSpacing; // Horizontal and vertical spacing between within elements
  ...
  float
    TabMinWidthForCloseButton; // Minimum width for close button to appears on
                               // an unselected tab when hovered. Set to 0.0f to
                               // always show when hovering, set to FLT_MAX to
  ...
  ImVec2
    DisplaySafeAreaPadding; // If you cannot see the edges of your screen (e.g.
...
struct ImGuiIO
{
  ...
  const char*
    LogFilename; // = "imgui_log.txt"// Path to .log file (default parameter to
  ...
  bool
    ConfigMacOSXBehaviors; // = defined(__APPLE__) // OS X style: Text editing
                           ...
                           // uses Cmd/Super instead of Ctrl (was called
  ...
  bool ConfigWindowsMoveFromTitleBarOnly; // = false       // [BETA] Set to true
                                          ...
                                          // affected.
  float
    ConfigMemoryCompactTimer; // = 60.0f          // [BETA] Free transient
                              // windows/tables memory buffers when unused for
                              // given amount of time. Set to -1.0f to disable.
  ...
  ImVec2
    MousePos; // Mouse position, in pixels. Set to ImVec2(-FLT_MAX, -FLT_MAX) if
  ...
  IMGUI_API void AddInputCharacterUTF16(
    ImWchar16 c); // Queue new character input from an UTF-16 character, it can
                  // be a surrogate
  IMGUI_API void AddInputCharactersUTF8(
    const char* utf8_chars); // Queue new characters input from an UTF-8 string
...
static inline void
OpenPopupContextItem(const char* str_id = NULL, ImGuiMouseButton mb = 1)
{
...
static inline bool
BeginPopupContextWindow(const char* str_id,
                        ImGuiMouseButton mb,
                        bool over_items)
{
...
struct ImDrawList
{
  ...
  unsigned int
    _VtxCurrentIdx; // [Internal] generally == VtxBuffer.Size unless we are past
                    // 64K vertices, in which case this gets reset to 0.
  ...
  ImDrawVert*
    _VtxWritePtr; // [Internal] point within VtxBuffer.Data after each add
                  // command (to avoid using the ImVector<> operators too much)
  ImDrawIdx*
    _IdxWritePtr; // [Internal] point within IdxBuffer.Data after each add
                  // command (to avoid using the ImVector<> operators too much)
...
struct ImDrawData
{
  ...
  ImVec2
    FramebufferScale; // Amount of pixels for each unit of DisplaySize. Based on
  ...
  IMGUI_API void ScaleClipRects(const ImVec2& fb_scale)
    const; // Helper to scale the ClipRect field of each ImDrawCmd. Use if your
...
struct ImFontConfig
{
  ...
  bool
    FontDataOwnedByAtlas; // true     // TTF/OTF data ownership taken by the
  ...
  int OversampleH;  // 3        // Rasterize at higher quality for sub-pixel
                   ...
                   // for details.
  int
    OversampleV; // 1        // Rasterize at higher quality for sub-pixel
  ...
  const ImWchar*
    GlyphRanges; // NULL     // Pointer to a user-provided list of Unicode range
                 // (2 value per range, values are inclusive, zero-terminated
                 // list). THE ARRAY DATA NEEDS TO PERSIST AS LONG AS THE FONT
                 // IS ALIVE.
...
struct ImFontGlyphRangesBuilder
{
  ...
  IMGUI_API void BuildRanges(
    ImVector<ImWchar>* out_ranges) const; // Output new ranges
};
...
enum ImFontAtlasFlags_
{
  ...
  ImFontAtlasFlags_NoBakedLines =
    1 << 2 // Don't build thick line textures into the atlas (save a little
           ...
           // otherwise they will be rendered using polygons (more expensive for
           // CPU/GPU).
};
...
struct ImFontAtlas
{
  ...
  // data.
  IMGUI_API static const ImWchar*
  ...
  // font for you.
  unsigned char*
    TexPixelsAlpha8; // 1 component per pixel, each component is unsigned 8-bit.
                     // Total size = TexWidth * TexHeight
  unsigned int*
    TexPixelsRGBA32; // 4 component per pixel, each component is unsigned 8-bit.
...
struct ImFont
{
  ...
  ImVector<float>
    IndexAdvanceX; // 12-16 // out //            // Sparse. Glyphs->AdvanceX in
                   // a directly indexable way (cache-friendly for CalcTextSize
                   // functions which only this this info, and are often
