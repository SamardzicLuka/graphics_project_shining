...
auto
loadTexture(const char* texPath) -> unsigned int;
auto
loadCubemap(vector<std::string> sides) -> unsigned int;
void
moveLight(Camera_Movement smer);
...
main() -> int
{
  ...
  //    glfwSetInputMode(window, GLFW_CURSOR, GLFW_CURSOR_DISABLED);  // DA LI
  //    ZELIMO DA NAM SE KURSOR VIDI NA EKRANU
  ...
  // -------------------------
  Shader carpetShader("resources/shaders/texture.vs",
                      "resources/shaders/texture.fs");
  Shader lightSourceShader("resources/shaders/source.vs",
                           "resources/shaders/source.fs");
  Shader skyboxShader("resources/shaders/skybox.vs",
                      "resources/shaders/skybox.fs");
  Shader windowShader("resources/shaders/blending.vs",
  ...
  // KORISTIMO ONAJ EBO KOJI SMO RADILI NA DRUGOM CASU KADA SE PONAVLJAJU
  // VERTICES
  ...
  glBufferData(
    GL_ARRAY_BUFFER, sizeof(carpetVertices), carpetVertices, GL_STATIC_DRAW);
  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, carpetEBO);
  glBufferData(GL_ELEMENT_ARRAY_BUFFER,
               sizeof(carpetIndices),
               carpetIndices,
               GL_STATIC_DRAW);
  ...
  unsigned int cubeTexture =
    loadTexture(FileSystem::getPath("resources/textures/stone.jpg").c_str());
  unsigned int cubeSpecularMap = loadTexture(
    FileSystem::getPath("resources/textures/specular_cube.jpg").c_str());
  unsigned int transparentTexture =
    loadTexture(FileSystem::getPath("resources/textures/cave.png").c_str());

  // POD
  unsigned int carpetTexture =
    loadTexture(FileSystem::getPath("resources/textures/carpet.jpg").c_str());
  unsigned int carpetSpecularMap = loadTexture(
    FileSystem::getPath("resources/textures/specular_carpet.jpg").c_str());
  ...
  // MODEL
  unsigned int modelSpec = loadTexture(
    FileSystem::getPath("resources/objects/axe/basecolor.png").c_str());
  carpetShader.use();
  ...
  Model axe(FileSystem::getPath("resources/objects/axe/axeLP.fbx"));
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    std::sort(windows.begin(),
              windows.end(),
              [kamericaPosition = kamerica.Position](const glm::vec3& a,
                                                     const glm::vec3& b) {
    ...
    carpetShader.setFloat("spotLight.outerCutOff",
                          glm::cos(glm::radians(13.0f)));

    glm::mat4 view = glm::lookAt(
      kamerica.Position, kamerica.Position + kamerica.Front, kamerica.Up);
    ...
    modelSikire =
      glm::rotate(modelSikire, (float)glfwGetTime(), glm::vec3(1.0f));
    ...
    view = glm::mat4(glm::mat3(
      kamerica.GetViewMatrix())); // remove translation from the view matrix
...
processInput(GLFWwindow* window)
{
  ...
  if (glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS)
    kamerica.ProcessKeyboard(FORWARD,
                             deltaTime / 2.0); // POMERA SE KAMERA U KOM GLEDAMO
...
{
  if (firstMouse) {
    lastX = xpos; // AKO JE OO PRVO POMERANJE MISEM, POSTAVLJAMO POZICIJU NA
                  // KOJOJ SMO PRETHODNO BILI NA OVU TRENUTNU
    lastY = ypos;
    firstMouse = false;
  }

  float xoffset = xpos - lastX; // ZA KOLIKO SE POMERAMO
  float yoffset = lastY - ypos; // ZA KOLIKO SE POMERAMO; reversed since
  ...
  // DA NAM NE BI KAMERA RADILA BACKFLIP. ONO KAD SE POMERAMO NPR. NA GORE I KAD
  // STIGNEMO DO 90 STEPENI.
  ...
  kamerica.Front =
    glm::normalize(front); // OVDE POSTAVLJAMO KONACNO SMER GLEDANJA KAMERE
}
...
loadCubemap(vector<std::string> sides) -> unsigned int
{
  ...
  for (unsigned int i = 0; i < sides.size(); i++) {
    unsigned char* data =
      stbi_load(sides[i].c_str(), &width, &height, &nrChannels, 0);
    if (data) {
      ...
      std::cout << "Cubemap texture failed to load at path: " << sides[i]
                << std::endl;
      stbi_image_free(data);
    }
  }
